...
if [ "$SHED_BUILDMODE" == 'toolchain' ]; then
    ...
    install -v -Dm644 "${SHED_CONTRIBDIR}/shedmake.default" "${SHED_FAKEROOT}/tools/etc/shedmake.conf"
    ...
    install -v -m644 "${SHED_CONTRIBDIR}/shedmake.default" "${SHED_FAKEROOT}/etc/shedmake/shedmake.default"
fi
